...
PROVIDERS: dict[str, dict[str, str]] = {
    "groq": {"env": "GROQ_API_KEY", "base": "https://api.groq.com/openai/v1", "model": "llama-3.3-70b-versatile"},
    ...
    "nvidia": {"env": "NVIDIA_API_KEY", "base": "https://integrate.api.nvidia.com/v1", "model": "meta/llama-3.2-11b-vision-instruct"},
}
...
def get_default_backend(preferred: Optional[str] = None) -> Backend:
    _load_dotenv()
    if preferred and preferred in PROVIDERS and os.environ.get(PROVIDERS[preferred]["env"]):
        return RemoteBackend(preferred)
    for p in ["groq", "nvidia", "deepseek", "openai", "anthropic", "google", "xai"]:
        if p in PROVIDERS and os.environ.get(PROVIDERS[p]["env"]):
            return RemoteBackend(p)
    return RuleBackend()
