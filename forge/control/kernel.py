...
class ControlKernel:
    ...
    def _predict(self, step: Step) -> str:
        ...
        if step.op is Op.EXEC_SANDBOX:
            return f"exec_sandbox: execute {step.command or 'script'} in sandbox"
    ...
    def _dispatch(self, step: Step) -> ActionResult:
        ...
        if step.op is Op.EXEC_SANDBOX:
            return self.ledger.exec_sandbox(step.command or "", script_content=step.content)
    ...
    def _verify(self, step: Step, res: ActionResult) -> tuple[bool, str]:
        ...
        if step.op is Op.EXEC_SANDBOX:
            return bool(res.ok), f"sandbox executed ({res.note})"
