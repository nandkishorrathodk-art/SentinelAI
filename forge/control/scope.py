...
class Op(str, Enum):
    ...
    EXEC_SANDBOX = "exec_sandbox"  # arbitrary code execution inside isolated sandbox


# Which ops need an explicit grant before they can be called at all.
MUTATING_OPS = {
    Op.WRITE, Op.APPEND, Op.COPY, Op.MOVE, Op.MKDIR, Op.DELETE, Op.RUN_TESTS, Op.EXEC_SANDBOX,
}
...
def default_policy(guard: PathGuard, operator: str = "human") -> CapabilitySet:
    ...
    if electron:
        ...
        caps.grant(Op.EXEC_SANDBOX, [electron], operator, ttl_seconds=3600, budget=100,
                   note="sandboxed execution with stdout/stderr capture")
    return caps