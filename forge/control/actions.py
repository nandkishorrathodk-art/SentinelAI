...
class ActionLedger:
    ...
    def exec_sandbox(self, command: str, script_content: Optional[str] = None, timeout: int = 30) -> ActionResult:
        """Executes a command or script in the workspace sandbox.
        
        Captures real stdout, stderr, exit code, and execution time.
        """
        self._authorise(Op.EXEC_SANDBOX, self.guard.roots[0] if self.guard.roots else None)
        cwd = self.guard.roots[0] if self.guard.roots else "."
        script_path = None
        undo_fn = None
        
        if script_content:
            script_path = os.path.join(cwd, f"_temp_exec_{uuid.uuid4().hex[:8]}.py")
            ...
            def _cleanup_script():
                if os.path.exists(script_path):
                    try:
                        ...
                        pass
            undo_fn = _cleanup_script

        import sys
        py_exe = sys.executable or "python"
        if command.startswith("python "):
            argv = [py_exe] + command[7:].split()
        elif command.startswith("python3 "):
            argv = [py_exe] + command[8:].split()
        else:
            argv = command.split()
        ...
        try:
            ...
            note = f"exit {res.returncode} in {elapsed:.2f}s"
        except subprocess.TimeoutExpired:
            ok = False
            val = {"returncode": -1, "stdout": "", "stderr": f"Timeout after {timeout}s", "elapsed": timeout}
            note = "timeout"
        except Exception as exc:
            ok = False
            val = {"returncode": -1, "stdout": "", "stderr": str(exc), "elapsed": 0.0}
            note = str(exc)
        ...
        return self._journal(ActionResult(
            Op.EXEC_SANDBOX, ok, None, val,
            Undo(f"sandbox execution: {command[:40]}", fn=undo_fn),
            note=note
        ))
